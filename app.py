...
if datos.empty:
    ...
    if rendimientos.empty:
        ...
        def calcular_metricas(rendimientos):
            ...
            sharpe = media / volatilidad if volatilidad.all() != 0 else np.nan
        ...
        def optimizar_portafolio(rendimientos, objetivo="sharpe", rendimiento_objetivo=None):
            ...
            if objetivo == "sharpe":
                def objetivo_func(pesos):
                    ...
                    return -rendimiento / riesgo if riesgo != 0 else np.inf
            ...
            try:
                ...
                if not resultado.success:
                    st.warning(f"Optimización no exitosa: {resultado.message}")
                return resultado.x
            except Exception as e:
                st.error(f"Error durante la optimización: {e}")
                return np.zeros(num_activos)
